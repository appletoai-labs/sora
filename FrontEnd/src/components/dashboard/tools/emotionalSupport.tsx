...
const Button: FC<ButtonProps> = ({ className, variant = "default", onClick, children, disabled }) => {
    const baseStyles = "px-4 py-2 rounded font-semibold transition flex items-center justify-center gap-2"
    const variantStyles =
        variant === "ghost"
    ...
    return (
        <button className={`${baseStyles} ${variantStyles} ${className}`} onClick={onClick} disabled={disabled}>
            {children}
        </button>
    )
}
...
const SelectItem: FC<{ value: string; children: React.ReactNode; onSelect: (value: string) => void }> = ({
    value,
    children,
    onSelect,
}) => (
    ...
    return (
        <div className={`relative ${className}`}>
            ...
            <div className="relative h-3 bg-gray-600 rounded-full">
                <div className="absolute h-full bg-sora-teal rounded-full transition-all" style={{ width: `${percentage}%` }} />
                <input
                    type="range"
                    min={min}
                    max={max}
...
const saveToRecentActivities = (activity: any) => {
    try {
        const existingActivities = JSON.parse(localStorage.getItem("sora_recent_activities") || "[]")
        const newActivity = {
            ...activity,
...
const StrategyExecutionModal: FC<StrategyExecutionModalProps> = ({ isOpen, onClose, strategy }) => {
    ...
    const handleComplete = () => {
        ...
        const activity = {
            ...
            description: `Successfully completed coping strategy for ${strategy.emotion || "emotional"} feelings`,
                ...
                {!isCompleted ? (
                            ...
                            <div className="flex items-center justify-between mb-2">
                                ...
                                {timeLeft > 0 && (
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-lg font-bold text-sora-teal">{timeLeft}s</span>
                                    </div>
                                )}
                            </div>
                        ...
                        <div className="text-center">
                            <div className="flex justify-center gap-2 mb-2">
                                {strategy.steps.map((_, index) => (
                                    <div
                                        key={index}
                                        className={`w-3 h-3 rounded-full ${index === currentStep ? "bg-sora-teal" : index < currentStep ? "bg-green-500" : "bg-gray-600"
                                            }`}
                                    />
                                ))}
                            </div>
                            <p className="text-sm text-gray-400">
                                Time needed: {strategy.timeNeeded} • Difficulty: {strategy.difficulty}
                            </p>
                        </div>
                    </div>
                ) : (
                    <div className="space-y-6">
                        <div className="text-center">
                            ...
                            <h4 className="text-xl font-bold text-white mb-2">Strategy Completed!</h4>
                            <p className="text-gray-300">How effective was this strategy for you?</p>
                        </div>

                        <div>
                            <label className="block text-white font-medium mb-3">Rate Effectiveness (1-5 stars)</label>
                            <div className="flex justify-center gap-2 mb-4">
                                {[1, 2, 3, 4, 5].map((rating) => (
                                    <button key={rating} onClick={() => setEffectiveness(rating)} className="p-1">
                                        <Star
                                            className={`w-8 h-8 ${rating <= effectiveness ? "text-yellow-400 fill-current" : "text-gray-400"
                                                }`}
                                        />
                                    </button>
                                ))}
                        ...
                        <div className="flex gap-3">
                            <Button onClick={handleComplete} className="flex-1" disabled={effectiveness === 0}>
                ...
                <div className="space-y-4">
                    <div className="bg-sora-muted/50 p-4 rounded-lg">
                        <div className="flex items-center justify-between mb-2">
                            ...
                            {timeLeft > 0 && <span className="text-2xl font-bold text-sora-teal">{timeLeft}</span>}
                        </div>
                        <p className="text-white">{exercise.steps[currentStep]}</p>
                    </div>

                    <div className="flex gap-3">
                        <Button onClick={isActive ? () => setIsActive(false) : startExercise} className="flex-1">
...
const StrategyCard: FC<StrategyCardProps> = ({ strategy, borderColor, iconColor, icon, onTryStrategy, emotion }) => {
    ...
    return (
        <Card className={`bg-gradient-to-br from-sora-card to-sora-muted border-l-4 ${borderColor} p-6`}>
            <div className="flex items-center justify-between mb-4">
                <div className={`w-12 h-12 ${iconColor} rounded-full flex items-center justify-center`}>{icon}</div>
                <div className={`px-3 py-1 rounded-full border text-sm font-medium ${getDifficultyColor(strategy.difficulty)}`}>
...
const EmotionalSupport: FC = () => {
    ...
    const [activeView, setActiveView] = useState<"main" | "strategies" | "strategiesResult">("main")
    ...
    const emotions = [
        { id: "anxious", label: "Anxious", emoji: "😰", color: "from-blue-500/20 to-blue-600/20 border-blue-400" },
        { id: "sad", label: "Sad", emoji: "😢", color: "from-sora-teal/20 to-sora-teal/30 border-sora-teal" },
        {
            id: "overwhelmed",
            label: "Overwhelmed",
            emoji: "😵",
        ...
        { id: "lonely", label: "Lonely", emoji: "😔", color: "from-gray-500/20 to-gray-600/20 border-gray-400" },
        { id: "angry", label: "Angry", emoji: "😡", color: "from-red-500/20 to-red-600/20 border-red-400" },
    ]
    ...
    const handleEmotionClick = (emotionId: string) => {
        ...
        const activity = {
            ...
            description: `Selected ${emotions.find((e) => e.id === emotionId)?.label} as current emotion`,
            ...
            <div className="max-w-6xl mx-auto">
                ...
                <div className="text-center mb-12">
                    ...
                    <p className="text-gray-300 text-lg max-w-3xl mx-auto">
                        AI-powered emotional support with therapeutic techniques designed for neurodivergent minds.
                    </p>
                </div>
                ...
                <div className="bg-gradient-to-br from-sora-card to-sora-muted border border-sora-teal/20 rounded-lg p-8 mb-8">
                    <h2 className="text-2xl font-bold text-white mb-4 text-center">How are you feeling right now?</h2>
                    <p className="text-gray-300 text-center mb-8">Select what best describes your current emotional state:</p>
                    ...
                    <div className="grid md:grid-cols-3 gap-6">
                        {exercises.map((exercise, index) => (
                            <Card key={index} className="bg-sora-muted/50 border border-sora-teal/20 p-6">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    <Card className="bg-gradient-to-br from-sora-card to-sora-muted border-l-4 border-l-sora-teal p-6">
                        ...
                        <p className="text-gray-300 text-sm mb-4">
                            Chat with our AI therapist trained in CBT, DBT, and mindfulness techniques. Get personalized emotional
                    ...
                    <Card className="bg-gradient-to-br from-sora-card to-sora-muted border-l-4 border-l-green-400 p-6">
                        ...
                        <p className="text-gray-300 text-sm mb-4">
                            Get personalized coping techniques based on your current emotions and triggers. Evidence-based strategies
                            adapted for neurodivergent minds.
                        </p>
                ...
                <Card className="bg-gradient-to-br from-sora-card to-sora-muted border border-sora-teal/20 p-8">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h2 className="text-2xl font-bold text-sora-teal">Evidence-Based Support</h2>
                    </div>
                    <p className="text-gray-300 mb-6">
                        SORA uses evidence-based therapeutic approaches adapted for neurodivergent individuals:
                    </p>
                    <div className="grid md:grid-cols-2 gap-6">
                        <div className="flex items-start gap-3">
                            <Brain className="w-6 h-6 text-sora-teal mt-1" />
                            <div>
                                <h3 className="text-white font-semibold mb-1">Cognitive Behavioral Therapy (CBT)</h3>
                                <p className="text-gray-300 text-sm">
                                    Identify and challenge negative thought patterns, develop healthier thinking habits.
                                </p>
                            </div>
                        </div>
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <h3 className="text-white font-semibold mb-1">Dialectical Behavior Therapy (DBT)</h3>
                                <p className="text-gray-300 text-sm">
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-gray-300 text-sm">
                                    Present-moment awareness and self-compassion practices adapted for neurodivergent minds.
                                </p>
                            </div>
                        </div>
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <h3 className="text-white font-semibold mb-1">Validation Therapy</h3>
                                <p className="text-gray-300 text-sm">
                                    Feel truly heard and understood with deep emotional validation and support.
                                </p>
                            </div>
                        </div>
                    </div>
        ...
        <div className="relative min-h-screen bg-gradient-to-br from-sora-dark via-gray-900 to-sora-dark p-6">
            ...
            <div className={`max-w-4xl mx-auto ${isLoading ? "blur-sm" : ""}`}>
                ...
                <div className="text-center mb-12">
                    ...
                    <p className="text-gray-300 text-lg max-w-3xl mx-auto">
                        Evidence-based techniques adapted for neurodivergent minds to help you navigate challenging emotions.
                    </p>
                </div>

                <Card className="bg-gradient-to-br from-sora-card to-sora-muted border border-sora-teal/20 p-8">
                    <h2 className="text-2xl font-bold text-white mb-4">Tell Me How You're Feeling</h2>
                    <p className="text-gray-300 mb-6">
                        Share your current emotional state so I can suggest the most helpful strategies for you right now.
                    </p>
                    ...
                    <div className="space-y-6">
                        <div>
                            ...
                            {fromEmotionCard ? (
                                ...
                                <Select value={selectedEmotion} onValueChange={setSelectedEmotion}>
                                    ...
                                    <SelectItem value="Anxious/Worried" onSelect={setSelectedEmotion}>
                                        Anxious/Worried
                                    </SelectItem>
                                    <SelectItem value="Sad/Down" onSelect={setSelectedEmotion}>
                                        Sad/Down
                                    </SelectItem>
                                    <SelectItem value="Overwhelmed/Stressed" onSelect={setSelectedEmotion}>
                                        Overwhelmed/Stressed
                                    </SelectItem>
                                    <SelectItem value="Angry/Frustrated" onSelect={setSelectedEmotion}>
                                        Angry/Frustrated
                                    </SelectItem>
                                    <SelectItem value="Lonely/Isolated" onSelect={setSelectedEmotion}>
                                        Lonely/Isolated
                                    </SelectItem>
                                    <SelectItem value="Confused/Uncertain" onSelect={setSelectedEmotion}>
                                        Confused/Uncertain
                                    </SelectItem>
                                    <SelectItem value="Exhausted/Drained" onSelect={setSelectedEmotion}>
                                        Exhausted/Drained
                                    </SelectItem>
                                    <SelectItem value="Rejected/Misunderstood" onSelect={setSelectedEmotion}>
                        ...
                        <div>
                            <label className="block text-white font-medium mb-3">How intense is this feeling? (1-10)</label>
                            <Slider value={intensity} onValueChange={setIntensity} max={10} min={1} step={1} className="mb-4" />
                        </div>

                        <div>
                            <label className="block text-white font-medium mb-3">What triggered this feeling? (optional)</label>
                            <textarea
                                value={triggers}
                ...
                <div className="max-w-6xl mx-auto">
                    ...
                    <div className="bg-gradient-to-r from-sora-teal to-sora-teal/80 rounded-lg p-6 mb-8">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <h2 className="text-xl font-bold text-sora-dark">Strategies for {emotionLabel} Feelings</h2>
                        </div>
                        <p className="text-sora-dark">
                            These techniques are specifically chosen for your current emotional state. Start with the ones that feel
                            most manageable.
                        </p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                        {quickAccessTools.map((tool, index) => (
                            <Card key={index} className="bg-sora-muted/50 border border-sora-teal/20 p-4 text-center">
                    ...
                    <div className="grid lg:grid-cols-2 gap-8 mb-8">
                        {strategies.map((strategy, index) => {
                            ...
                            const iconColors = ["bg-green-400", "bg-blue-400", "bg-purple-400", "bg-orange-400"]
                            const icons = [
                    ...
                    <Card className="bg-gradient-to-br from-sora-card to-sora-muted border border-sora-teal/20 p-8">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <h2 className="text-2xl font-bold text-sora-teal">Need More Support?</h2>
                        </div>
                        ...
                        <div className="grid md:grid-cols-3 gap-6">
                            <div className="text-center">
                                ...
                                <p className="text-gray-300 text-sm mb-4">Personalized emotional support and guidance</p>
                                <Button onClick={() => navigate("/app/chat")} className="w-full">
                                    Chat Now
                                </Button>
                            </div>
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-gray-300 text-sm mb-4">Understand your emotional trends over time</p>
                                <Button onClick={() => navigate("/app/dashboard")} variant="outline" className="w-full">
                                    View Insights
                                </Button>
                            </div>
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-gray-300 text-sm mb-4">Connect with others who understand</p>
    ...
    return (
        <div className="relative">
            ...
            <StrategyExecutionModal
                ...
                strategy={selectedStrategy || { id: "", title: "", description: "", steps: [], timeNeeded: "", difficulty: "" }}
            />
        </div>
    )
}
