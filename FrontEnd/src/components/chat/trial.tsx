...
export const Trialchat = () => {
    ...
    const initChat = async () => {
      ...
      try {
        ...
        if (wasViewingPast) {
          ...
          await handleNewChat() // This will trigger global loading
        }
      } catch (error) {
  ...
  const handleNewChat = async () => {
    ...
    const createNewSession = async () => {
      try {
        ...
        setPreviousResponseId(previousResponseId) // This might need to be reset to null for a new chat
        syncLastSessionToDB()
      ...
      if (currentSessionId) {
        try {
          ...
          if (endErr.response?.data?.error === "Active chat session not found") {
            ...
            await createNewSession() // Create new session if current one not found
            return // Exit after creating new session
          } else {
            ...
            throw endErr // Re-throw to be caught by outer try-catch
          }
        }
      }
      await createNewSession() // Always create a new session if no current one or if ended successfully
    } catch (err) {
      toast({
  ...
  const handleSendMessage = async (text: string) => {
    ...
    try {
      ...
      if (!sessionId || sessionId === "null") {
        // If no session, create one. This part will trigger global loading.
        startGlobalLoading()
        try {
          const res = await axios.post(
            `${API_BASE}/chatproxy/chat/session`,
  ...
  const handleViewSummary = async () => {
    startGlobalLoading() // Replaced setLoadingSummary(true)
    ...
    try {
      ...
      endGlobalLoading() // Replaced setLoadingSummary(false)
    }
  }
          ...
          <div className="w-full px-3 sm:px-6 space-y-2 sm:space-y-3 pb-4 sm:pb-6">
            ...
            {!isViewingPastSession && (
              <EnhancedChatInput
                ...
                disabled={isTyping || isGlobalLoading} // Disable input during global loading
