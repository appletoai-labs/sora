          ...
          {functions.map((func, index) => (
            <Card key={index} className="group hover:shadow-soft transition-all duration-300 border-none bg-gradient-card">
              <CardHeader className="space-y-4">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-calm-purple/20 to-soft-blue/20 flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                  <func.icon className="w-6 h-6 text-calm-purple" />
                </div>
                <div>
                  <CardTitle className="text-xl mb-2 group-hover:text-calm-purple transition-colors">
                    {func.title}
                  </CardTitle>
                ...
                <ul className="space-y-2">
                  {func.features.map((feature, featureIndex) => (
                    <li key={featureIndex} className="flex items-start gap-2 text-sm text-muted-foreground">
                      <div className="w-1.5 h-1.5 rounded-full bg-gentle-green mt-2 flex-shrink-0" />
                      <span>{feature}</span>
                    </li>
                  ))}
                </ul>
